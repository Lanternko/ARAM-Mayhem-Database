...
class ClassicResearchPageTests(unittest.TestCase):
    ...
    def test_public_page_contains_interactive_research_surfaces_and_metadata(self) -> None:
        ...
        self.assertIn("aram-mayhem-site-theme", page)
        self.assertIn("setAttribute('data-theme',theme)", page)
        ...
        self.assertIn("border:0;border-radius:10px", page)
        self.assertIn("inset:0;z-index:3;\nborder:4px solid var(--tier-color", page)
        self.assertIn("data-experimental-tier-detail", page)
        self.assertIn("detail-head-rate", page)
        self.assertIn("has-open-detail", page)
        self.assertNotIn("classic-mode-label", page)
