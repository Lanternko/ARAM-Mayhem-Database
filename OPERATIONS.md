...
- `POST /api/feedback` 是只寫入的功能回饋入口，資料獨立保存於 `ARAM_FEEDBACK_DB`（預設 `data/site/feedback.db`），沒有公開讀取 API。正式 API task 必須保留該路徑在私有資料目錄，並可用 `ARAM_FEEDBACK_RATE_LIMIT_PER_HOUR` 調整每個客戶端的限流；回饋內容需透過受控的本機資料庫流程整理，不直接自動轉成公開 Issue。 Discord 通知由 API lifespan 啟動單一背景 worker；設定 `ARAM_FEEDBACK_DISCORD_WEBHOOK_FILE` 指向 private UTF-8 webhook 檔，或使用 `ARAM_FEEDBACK_DISCORD_WEBHOOK_URL`（不可提交 URL）。每筆新回饋與 `feedback_discord_outbox` 同 transaction 保存；兩秒輪詢、120 秒 lease、10 秒 HTTP timeout，成功紀錄 Discord message ID，失敗依退避／429 retry-after 重試。舊版回饋不自動補送；honeypot 不排入。投遞是 at-least-once，回應遺失或送達後程序崩潰可能重複，依 F 編號辨識。Webhook 與 Discord response body 不進 log；用 outbox 的 `attempts`、`last_error`、`delivered_at` 查狀態。本站不寄 Email，表單 Email 只供站方回覆，會隨已同意的回饋轉送私有 Discord 頻道。Webhook 設定需保存在 API host 的 ignored private path，變更後只重啟 ARAMMeta API。
