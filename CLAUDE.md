...
- **Never split games.db by queue into separate tables** — 所有 queue（2400 Mayhem / 450 ARAM / 2450 大混戰經典風 / 4310 經典）共用單一 `games` 表，靠 `queue_id` 欄位＋`idx_games_queue_patch_created` 過濾；42+ 檔案、`merge-db`、`game_id` 去重、`crawl_seen`/`crawl_queue` frontier 全假設單表，分表會全破。零值不是分表理由：`participants_json` 是稀疏 JSON，模式專屬欄位（vision/wards/jungle 等，見 `_SPARSE_ZERO_STAT_KEYS`）值為 0 時**根本不寫入**，ARAM 記錄天生沒有那些鍵。讀這些欄位一律 `.get(key, 0)`。
- **Never join 4310 champion ids without `base_champion_id()`** — 經典（queue 4310, gameMode JADE, map 453）每隻英雄是獨立 `Jade_*` id＝`60000+base`（60001=Annie），且只有 60 隻；直接對 metadata join 會讓 60 隻全變 unknown 被丟。用 `gamedata.base_champion_id()`（非 Jade id 原樣通過）。經典要上網站是**另立 pipeline**（有分路/視野/野區，現有 position-free 模型不適用）— 拆前端/分析層，不是拆資料表。
