...
def render_html(
    ...
    *,
    ...
    parts.append(
        ...
        "data-i18n-zh='遊玩建議：估計勝率主要看英雄強度（單獨勝率）；"
        "池內決勝常看默契（搭檔相性）；"
        "傷害比例與前排／開戰／清兵等組成是較小的微調。"
        "先抓強勢英雄，再拼搭配，最後補缺口。' "
        "data-i18n-zh-cn='游玩建议：估计胜率主要看英雄强度（单独胜率）；"
        "池内决胜常看默契（搭档相性）；"
        "伤害比例与前排／开战／清兵等组成是较小的微调。"
        "先抓强势英雄，再拼搭配，最后补缺口。' "
        "data-i18n-en='Tips: est. WR is driven mostly by champ strength (solo WR); "
        "within a pool, synergy (pair chemistry) often decides the ranking; "
        "damage mix and front / engage / wave are smaller composition tweaks. "
        "Grab strong champs first, then synergy, then fill gaps.'>"
        "遊玩建議：估計勝率主要看英雄強度（單獨勝率）；"
        "池內決勝常看默契（搭檔相性）；"
        "傷害比例與前排／開戰／清兵等組成是較小的微調。"
        "先抓強勢英雄，再拼搭配，最後補缺口。"
        "</span>"
