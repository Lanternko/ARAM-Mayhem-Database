...
def icon_svg() -> str:
    pips = "".join(
        f'  <circle cx="{x}" cy="{y}" r="{PIP_RADIUS}" fill="{BACKGROUND}"/>\n'
        for x, y in PIPS
    )
    return (
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">\n'
        f'  <rect x="3" y="3" width="58" height="58" rx="12" fill="{FOREGROUND}"/>\n'
        + pips + '</svg>\n'
    )
...
def icon_image(size: int):
    """Supersample the die with transparent padding and two large dark pips."""
    ...
    draw.rounded_rectangle((3 * scale, 3 * scale, 61 * scale, 61 * scale),
                           radius=12 * scale, fill=FOREGROUND)
    for x, y in PIPS:
